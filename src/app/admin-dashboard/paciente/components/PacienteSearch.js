...
import { fetchWithAuth } from "@/api/api"; // Importa la función desde lib/api
...
export default function PacienteSearch({ onSelectPaciente }) {
  ...
  const fetchPacientes = async () => {
    ...
    try {
      const apiUrl = `/api/paciente/get/${query}`;
      ...
      <div className="w-full max-w-lg bg-white p-6 rounded-lg shadow-md">
        ...
        {pacientes.map((paciente) => (
          <div
            ...
            className="border p-4 rounded-lg shadow-sm bg-gray-50 mt-4"
          >
